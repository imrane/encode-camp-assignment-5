...
async function initContracts() {
  ...
  token = lotteryTokenContractFactory.attach(tokenAddress).connect(ethers.provider) as LotteryToken;
}
...
async function checkState() {
  ...
  console.log(`The last block timestamp was 
    ${new Date((lastBlock.timestamp * 1000).toLocaleString())} 
    and the lottery is only going to accept bets until 
    ${new Date((closingTimestamp).toNumber() * 1000)}`);
}
